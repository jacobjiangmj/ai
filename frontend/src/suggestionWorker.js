...
function minimax(sourceBoard, sideToMove, depth, alpha, beta, maximizingSide, deadline) {
  ...
  if (sideToMove === maximizingSide) {
    ...
    for (const move of orderedMoves) {
      ...
      if (result.timedOut || beta <= alpha) {
        return { score: bestScore, timedOut: result.timedOut }
      }
    }
...
function getSuggestionDepth(aiDifficulty, suggestionStrength) {
  ...
  const base = suggestionDepthConfig[aiDifficulty] || 4
  return base + (suggestionStrengthDepthBonus[suggestionStrength] || 0)
}

function getRankedMovesWithBudget(board, side, aiDifficulty, suggestionStrength, suggestionStyle, limit = 3, maxMs = 5000) {
  const legal = getAllLegalMoves(board, side).sort(compareMoves)
  if (!legal.length) {
    return { moves: [], timedOut: false }
  }

  const deadline = Date.now() + maxMs
  const searchDepth = Math.max(getSuggestionDepth(aiDifficulty, suggestionStrength) - 1, 0)
  const scoredMoves = []

  for (let index = 0; index < legal.length; index += 1) {
    const move = legal[index]
    const nextBoard = applyMoveToBoard(board, move)
    const quickScore = getQuickSuggestionScore(move, side, nextBoard, suggestionStyle)
    scoredMoves.push({ move, score: quickScore, nextBoard })
  }
  ...
  for (let index = 0; index < deepCandidates.length; index += 1) {
    if (searchDepth <= 0 || Date.now() >= deadline) {
      timedOut = true
      break
    }

    const entry = deepCandidates[index]
...
self.onmessage = (event) => {
  const { board, side, aiDifficulty, suggestionStrength, suggestionStyle, limit, maxMs } = event.data
  const result = getRankedMovesWithBudget(board, side, aiDifficulty, suggestionStrength, suggestionStyle, limit, maxMs)
